...
const createPickup = (arr) => {
    return `
        
        ${arr.map((item) => {
        return `
            <div class="py-4 flex flex-col md:flex-row mt-4" >
                ...
                <div  class="bg-[#1c3a33] text-center w-full md:w-full flex justify-center items-center">
                    <div class="max-w-md mx-auto md:max-w-2xl text-white">
                        <div class="p-4 text-start ">
                            <div class="text-2xl md:text-4xl font-bold"> ${item.title}</div>
                            <p class="py-5 mt-2 text-[19px] md:text-xl"> ${item.text}</p>
                            <div class="flex flex-col items-start  ">
                                <button class="bg-green-700 text-white text-[16px] font-medium m-4 p-2 px-5 rounded-full"><a href="">${item.button}</a></button>
                            </div>
                        </div>
                    </div>
                </div>
...
class PickUpCard extends HTMLElement {
    ...
    connectedCallback() {
        this.innerHTML = createPickup(pickupCardArr)
    }
}
